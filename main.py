...
def eliminar_equipo(request: Request, id_equipo: int = Path(...), db: Session = Depends(get_db)):
    ...
    try:
        # Eliminar detalles de PC si existen
        detalle_pc = db.query(PcDetalle).filter(PcDetalle.id_equipo == id_equipo).first()
        if detalle_pc:
            # Eliminar gráfica dedicada si existe
            grafica = db.query(GraficaDedicada).filter(GraficaDedicada.id_pc == detalle_pc.id_pc).first()
            if grafica:
                db.delete(grafica)
            db.delete(detalle_pc)
        ...
        db.rollback()
        request.session["error"] = f"❌ Error al eliminar equipo: {str(e)}"
...
from models import Equipo, TipoDispositivo, PcDetalle, GraficaDedicada, Periferico
...
async def actualizar_detalles_pc(
    ...
    try:
        ...
        # Procesar periféricos (se eliminan y se vuelven a insertar)
        db.query(Periferico).filter(Periferico.id_equipo == id_equipo).delete()
